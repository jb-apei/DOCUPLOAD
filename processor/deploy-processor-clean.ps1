...
.EXAMPLE
    .\deploy-processor-clean.ps1 -Version "v1.0.0"
#>
...
echo $ACR_PASSWORD | docker login "$ACR_NAME.azurecr.io" -u $ACR_NAME --password-stdin | Out-Null
...
docker push "$ACR_NAME.azurecr.io/$IMAGE_NAME`:$Version" | Out-Null
docker push "$ACR_NAME.azurecr.io/$IMAGE_NAME`:latest" | Out-Null
...
if ($appExists) {
    ...
    az containerapp update --name $APP_NAME --resource-group $RESOURCE_GROUP --image "$ACR_NAME.azurecr.io/$IMAGE_NAME`:$Version" | Out-Null
    
    # Update secrets
    az containerapp secret set --name $APP_NAME --resource-group $RESOURCE_GROUP --secrets "service-bus-connection=$SERVICE_BUS_CONN" "storage-key=$STORAGE_KEY" | Out-Null
    
    # Update environment variables
    az containerapp update --name $APP_NAME --resource-group $RESOURCE_GROUP --set-env-vars "SERVICE_BUS_CONNECTION_STRING=secretref:service-bus-connection" "SERVICE_BUS_QUEUE_NAME=$QUEUE_NAME" "AZURE_STORAGE_ACCOUNT_NAME=$STORAGE_ACCOUNT" "AZURE_STORAGE_ACCOUNT_URL=$STORAGE_URL" "AZURE_STORAGE_ACCOUNT_KEY=secretref:storage-key" "SOURCE_CONTAINER_NAME=$SOURCE_CONTAINER" "PROCESSED_CONTAINER_NAME=$PROCESSED_CONTAINER" | Out-Null
...
# Get the app details
Write-Host ""
Write-Host "Retrieving app details..." -ForegroundColor Yellow
$appDetails = az containerapp show --name $APP_NAME --resource-group $RESOURCE_GROUP | ConvertFrom-Json
...
Write-Host "Status: $($appDetails.properties.provisioningState)" -ForegroundColor Cyan
Write-Host ""
